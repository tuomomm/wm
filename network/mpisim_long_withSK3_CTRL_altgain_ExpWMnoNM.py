#cp mpisim_long_withSK3_CTRL_altgain_ExpWM_verbose.py mpisim_long_withSK3_CTRL_altgain_ExpWMnoNM_verbose.py
import numpy
...
h.allfiles=allfiles = "geom.hoc pyinit.py mpisim_long_withSK3_CTRL_altgain.py"
h.simfiles=simfiles = "pyinit.py geom_withSK3.py mpisim_long_withSK3_CTRL_altgain.py"
h("runnum=1")
runnum = 1.0
h.datestr=datestr = "15dec31"
h.output_file=output_file = "data/sim_net_long_withSK3_CTRL_altgain"
...
from geom_long_withSK3_CTRL_altgain import *
from nqs import *
import random
from pylab import *
...
def backupcfg (simstr):
  ...
  try:
    os.system('cp ' + fcfg + ' ' + fout) # fcfg created in geom_long_withSK3_CTRL_altgain.py via conf.py
  except:
    pass
...
def setupWMInputs ():
  ...
  if False: #os.path.exists(dconf['nqm']): # read nqm if it exists
    nqm = h.NQS(dconf['nqm'])
  ...
  #Tuomo: Comments by chatgpt.
  def createNetStims (vid,rate,w,startt,endt,seed=1234,syn='Adend3AMPA'):  # create external NetStim inputs to cells listed in vid. 
    global rnnsl,rnncl,rnrds,rnseed # global lists storing NetStim objects, NetCons, RNGs, and RNG seeds
    rnnsl.append( [] ) # add a new sublist to store NetStim objects created in this call
    rnncl.append( [] ) # add a new sublist to store NetCon connections created in this call
    rnrds.append( [] ) # add a new sublist to store Random() generators used for NetStim noise
    rnseed.append( [] ) # add a new sublist to store seeds used for reproducibility

    for idx in vid: # iterate over the list/vector of target cell gids
      if not pc.gid_exists(idx): continue # skip if this MPI process does not own the cell with gid idx

      if verbose: print('idx is ' , idx, ' dlid = ' , dlids[idx], ' type is ' , CTYP[lctyID[int(idx)]]) 
      # debug print: show gid, local cell index, and cell type
      cell = ce[dlids[idx]] # retrieve the actual cell object from the global cell list using the local index
      if ice(cell.ty) and syn.count('mGLUR') > 0: continue 
      # if the cell is inhibitory (checked by ice()) and the requested synapse is mGluR, skip (not supported)
      slist, sidx = getslist(cell, syn) 
      # find all sections in the cell that contain the requested synapse type
      # slist = list of sections containing that synapse
      # sidx = index of that synapse type in each section's synapse list

      if sidx == -1: 
      # if the synapse type was not found in dendrites (special case, often soma synapse)
        ns = h.NetStim() # create a NetStim spike generator
        ns.start = startt # set the time when spikes begin (ms)
        ns.number = (endt-startt) * rate / 1e3 
        # total number of spikes to emit = duration * rate (converted from Hz to spike count)
        if verbose: print('createNetStims:',startt,endt,ns.number,rate,w )
        # optional debug print showing stimulus parameters
        ns.interval = 1e3 / rate 
        # mean interval between spikes in ms (inverse of rate)
        ns.noise = 1 
        # enable stochastic spike timing (Poisson-like spike train)
        rnnsl[-1].append(ns) 
        # store this NetStim in the last entry of the global NetStim list
        if ice(cell.ty): 
        # if the target cell is inhibitory
          if syn.count('AMPA'): 
            nc = h.NetCon(ns,cell.__dict__['somaAMPA'].syn) 
            # connect NetStim to AMPA synapse located on the soma
          elif syn.count('NMDA'): 
            nc = h.NetCon(ns,cell.__dict__['somaNMDA'].syn) 
            # connect NetStim to NMDA synapse on soma
          if IsLTS(cell.ty): 
            nc.weight[0] = w / 4.0 
            # scale synaptic weight for LTS interneurons
          else:
            nc.weight[0] = w / 4.0 
            # same weight scaling for other inhibitory neurons
        else: 
        # if the cell is excitatory
          nc = h.NetCon(ns,cell.__dict__[syn].syn) 
          # connect NetStim to the specified synapse on the cell
          nc.weight[0] = w 
          # assign synaptic weight
        nc.delay = h.dt * 2 
        # set synaptic transmission delay (twice the integration timestep)
        rnncl[-1].append(nc) 
        # store the NetCon in the global connection list
        rds = h.Random() 
        # create a NEURON random number generator object
        rds.negexp(1) 
        # configure it to generate exponentially distributed intervals (Poisson process)
        rds.MCellRan4(seed*idx,seed*idx) 
        # initialize RNG using MCellRan4 algorithm with seed derived from cell gid
        ns.noiseFromRandom(rds) 
        # attach this random generator to the NetStim noise mechanism
        rnrds[-1].append(rds) 
        # store RNG so it persists (prevents garbage collection)
        rnseed[-1].append(seed*idx) 
        # record the seed used for reproducibility
      else: 
      # case where the synapse exists in dendritic sections
        tmpseed = seed 
        # start with base seed value for generating unique seeds
        for sec in slist: 
        # iterate over all sections containing this synapse type
          llsy = cell.dsy[sec] 
          # retrieve the list of synapse objects in this section
          for lsy in llsy: 
          # iterate through synapses within the section
            ns = h.NetStim() 
            # create a new spike generator
            ns.start = startt 
            # set spike start time
            ns.number = (endt-startt) * rate / 1e3 
            ...
            # set spike interval
            ns.noise = 1 
            # enable stochastic firing
            rnnsl[-1].append(ns)
            # store NetStim
            nc = h.NetCon(ns,lsy[sidx].syn) 
            # connect NetStim to this dendritic synapse
            nc.delay = h.dt * 2 
            ...
            # initialize RNG using gid-based seed
            ns.noiseFromRandom(rds) 
            # link RNG to NetStim
            rnrds[-1].append(rds) 
            # store RNG
            ...
            # store seed used
            tmpseed += 1 
            # increment seed so each synapse gets different random sequence

    if verbose: vid.printf() 
    # if debugging, print the list of gids that received NetStim inputs

  #Tuomo: define a function to provide exponentially decaying input rate
  def createExpDecayStim(idx, startt, decayConst, rate, seed=1234, duration=100000.0, dt=1.0):
    """
    Generate exponentially decaying Poisson spikes using NEURON RNG.
    Returns VecStim and spike vector.
    """

    r = h.Random()
    r.Random123(seed, idx, 0)

    t = 0.0
    spikes = []

    while t < duration:
        lam = rate * exp(-t / decayConst)
        p = lam * dt / 1000.0

        if r.uniform(0,1) < p:
            spikes.append(startt + t)

        t += dt

    spikevec = h.Vector(spikes)
    try:
      print("createExpDecayStim: "+str(spikevec.size())+" spikes, seed="+str(seed)+", idx="+str(idx)+", startt="+str(startt))
    except:
      print("print passed")
    ...
    return stim, spikevec

  #Tuomo: Copied from createNetStims above, adjusted to use createExpDecayStim.
  #Called as "createNetStimsExpDecay(vid, rate, w, startt, decayConst, seed, syn)" from createNetStimsExpDecayFromNQ
  def createNetStimsExpDecay (vid,rate,w,startt,decayConst,seed=1234,syn='Adend3AMPA'):  # create external NetStim inputs to cells listed in vid. 
    global rnnsl,rnncl # global lists storing NetStim objects, NetCons
    rnnsl.append( [] ) # add a new sublist to store NetStim objects created in this call
    rnncl.append( [] ) # add a new sublist to store NetCon connections created in this call
    rnrds.append([]) #Tuomo: This wouldn't be needed, but later in the script the code expects this to be the same size as rnnsl
    rnseed.append([]) #Tuomo: This wouldn't be needed, but later in the script the code expects this to be the same size as rnnsl
    for idx in vid: # iterate over the list/vector of target cell gids
      if not pc.gid_exists(idx): continue # skip if this MPI process does not own the cell with gid idx
      if verbose: print('idx is ' , idx, ' dlid = ' , dlids[idx], ' type is ' , CTYP[lctyID[int(idx)]] )
      # debug print: show gid, local cell index, and cell type
      ...
      # slist = list of sections containing that synapse
      # sidx = index of that synapse type in each section's synapse list
      stim, spikevec = createExpDecayStim(idx, startt, decayConst, rate, seed)
      try:
        print("createExpDecayStim: "+str(spikevec.size())+" spikes, seed="+str(seed)+", idx="+str(idx)+", startt="+str(startt))
      except:
        print("print passed")
      rnnsl[-1].append(stim)
      rnnsl[-1].append(spikevec)  # keep vector alive
      rnrds[-1].append(None); rnrds[-1].append(None) #Add empty objects to keep the same size
      rnseed[-1].append(seed*idx); rnseed[-1].append(seed*idx)
      if sidx == -1: 
      # if the synapse type was not found in dendrites (special case, often soma synapse)
        if ice(cell.ty): 
        # if the target cell is inhibitory
          if syn.count('AMPA'): 
            nc = h.NetCon(stim,cell.__dict__['somaAMPA'].syn) 
            # connect NetStim to AMPA synapse located on the soma
          elif syn.count('NMDA'): 
            nc = h.NetCon(stim,cell.__dict__['somaNMDA'].syn) 
            # connect NetStim to NMDA synapse on soma
          if IsLTS(cell.ty): 
            nc.weight[0] = w / 4.0 
            # scale synaptic weight for LTS interneurons
        ...
        # if the cell is excitatory
          nc = h.NetCon(stim,cell.__dict__[syn].syn) 
          # connect NetStim to the specified synapse on the cell
          nc.weight[0] = w 
          # assign synaptic weight
        nc.delay = h.dt * 2 
  ...
  #Tuomo: Copied from createNetStimsFromNQ
  #Called as "createNetStimsExpDecayFromNQ(nqm,i,seed=(j+1)*(i+1)*12345,syn='Adend'+str(j)+'AMPA',wfctr=AMRWM)" from setStims
  def createNetStimsExpDecayFromNQ (nqm,row,seed=1234,syn='Adend3AMPA',wfctr=1.0,decayConst=16000.0,rateCoeff=3.3):
    nqm.tog("DB")
    vid = h.Vector()
    # If kExpWM exists in the table, override default
    try:
      rateCoeff = float(dconf['kExpWM'])
    except:
      print("dconf['kExpWM'] not found")
      pass
    rate = rateCoeff*nqm.getcol("rate").x[row] #Tuomo: How much larger is the activity in the beginning of the WM-like stimulus than at baseline
    w = nqm.getcol("w").x[row] * wfctr # wfctr is weight scaling factor
    vid.copy(nqm.get("vid",row).o[0])
    startt = float(nqm.getcol("startt").x[row]) # + tstart
    # If decayConst exists in the table, override default
    try:
      decayConst = float(dconf['decayConstExpWM'])
    except:
      print("dconf['decayConstExpWM'] not found")
      pass

    try:
      print("createNetStimsExpDecayFromNQ: rateCoeff = "+str(rateCoeff)+", rate = "+str(rate)+", startt="+str(startt)+", decayConst="+str(decayConst))
    except:
      print("print passed")

    createNetStimsExpDecay(vid, rate, w, startt, decayConst, seed, syn) #vid,rate,w,startt,endt,seed,syn)
  ...
  def setStims ():
    ...
    for i in range(sz):
      for j in lapicIDX:
        ...
        createNetStimsExpDecayFromNQ(nqm,i,seed=(j+1)*(i+1)*12345,syn='Adend'+str(j)+'AMPA',wfctr=AMRWM)
...
def myrun ():
  ...
  setupWMInputs() #Tuomo: setupWMInputs has been changed so that it doesn't include any of the previous AMPA, NMDA and mGLUR inputs but only the new AMPA inputs mimicking the WM-like increase in input firing rate
  h.stdinit()
