...
do
 ...
 do
  ...
  #Files where apical calcium is recorded
  if [ -f FR_sim_net${EXT}_seed${myseed}_withCaApic_higherres.mat ]
  then
      echo FR_sim_net${EXT}_seed${myseed}_withCaApic_higherres.mat exists
  else
      echo FR_sim_net${EXT}_seed${myseed}_withCaApic_higherres.mat does not exist
      ...
      echo """from pylab import *""" > simdat${EXT}_withCaApic_higherres.py
      cat simdat_saveFRandCaApic_higherres.py | sed "s/mpisim.py/mpisim${EXT}.py/" | sed "s/15dec31.1/sim_net${EXT}/" | sed "s/netcfg.cfg/netcfg${EXT}.cfg/" | sed "s/geom.py/geom${EXT}.py/" | sed "s/8,14/8,100/" | sed "s/lw=0.5/lw=0.25/" | sed "s/lwidth=1/lwidth=0.25/" | sed "s/lwidth=4/lwidth=1/" >> simdat${EXT}_withCaApic_higherres.py
      ...
      echo "python3 simdat${EXT}_withCaApic_higherres.py"
      python3 simdat${EXT}_withCaApic_higherres.py

      echo "mv FR_sim_net${EXT}_withCaApic_higherres.mat FR_sim_net${EXT}_seed${myseed}_withCaApic_higherres.mat"
      mv FR_sim_net${EXT}_withCaApic_higherres.mat FR_sim_net${EXT}_seed${myseed}_withCaApic_higherres.mat
  ...
  #Files where somatic calcium is recorded
  if [ -f FR_sim_net${EXT}_seed${myseed}_withCa_higherres.mat ]
  then
      echo FR_sim_net${EXT}_seed${myseed}_withCa_higherres.mat exists
  else
      echo FR_sim_net${EXT}_seed${myseed}_withCa_higherres.mat does not exist
      cp netcfg.cfg netcfg${EXT}.cfg
      ...
      echo """import numpy""" > mpisim${EXT}.py
      cat mpisim_withSK3_l5hzonly.py | sed "s/from geom_withSK3/from geom${EXT}/" | sed "s/if os.path.exists(dconf/if False: #os.path.exists(dconf/" | sed "s/mpisim.py/mpisim${EXT}.py/" | sed "s/geom.py/geom${EXT}.py/" | sed "s/15dec31.1/sim_net${EXT}/" >> mpisim${EXT}.py
      echo """from pylab import *""" > simdat${EXT}_withCa_higherres.py
      cat simdat_saveFRandCa_higherres.py | sed "s/mpisim.py/mpisim${EXT}.py/" | sed "s/15dec31.1/sim_net${EXT}/" | sed "s/netcfg.cfg/netcfg${EXT}.cfg/" | sed "s/geom.py/geom${EXT}.py/" | sed "s/8,14/8,100/" | sed "s/lw=0.5/lw=0.25/" | sed "s/lwidth=1/lwidth=0.25/" | sed "s/lwidth=4/lwidth=1/" >> simdat${EXT}_withCa_higherres.py

      echo "mpiexec -np 8 --bind-to core:overload-allowed nrniv -python -mpi mpisim${EXT}.py netcfg${EXT}.cfg tstop=100000 ${PARCHANGES[isim]} simstr=sim_net${EXT}"
      mpiexec -np 8 --bind-to core:overload-allowed nrniv -python -mpi mpisim${EXT}.py netcfg${EXT}.cfg tstop=100000 ${PARCHANGES[isim]} simstr=sim_net${EXT}

      echo "python3 simdat${EXT}_withCa_higherres.py"
      python3 simdat${EXT}_withCa_higherres.py

      echo "mv FR_sim_net${EXT}_withCa_higherres.mat FR_sim_net${EXT}_seed${myseed}_withCa_higherres.mat"
      mv FR_sim_net${EXT}_withCa_higherres.mat FR_sim_net${EXT}_seed${myseed}_withCa_higherres.mat
