...
do
 ...
 for isim in 0 1 2
 do
  EXT=_long_withSK3${EXTS[isim]}_altgain_withallNMDA
  ...
  then
      ...
      cat geom_withSK3_saveExtra2.py | sed "s/netcfg.cfg/netcfg${EXT}.cfg/" > geom${EXT}.py
      echo """import numpy""" > mpisim${EXT}.py
      cat mpisim_withSK3_l5hzonly_saveExtra2.py | sed "s/from geom_withSK3_saveExtra/from geom${EXT}/" | sed "s/if os.path.exists(dconf/if False: #os.path.exists(dconf/" | sed "s/mpisim.py/mpisim${EXT}.py/" | sed "s/geom.py/geom${EXT}.py/" | sed "s/15dec31.1/sim_net${EXT}/" >> mpisim${EXT}.py
      echo """from pylab import *""" > simdat${EXT}.py
      cat simdat_saveFRandMUAonly_withallNMDA_allowNonArray.py | sed "s/mpisim.py/mpisim${EXT}.py/" | sed "s/15dec31.1/sim_net${EXT}/" | sed "s/netcfg.cfg/netcfg${EXT}.cfg/" | sed "s/geom.py/geom${EXT}.py/" | sed "s/8,14/8,100/" | sed "s/lw=0.5/lw=0.25/" | sed "s/lwidth=1/lwidth=0.25/" | sed "s/lwidth=4/lwidth=1/" >> simdat${EXT}.py
      ...
      #Don't delete these files because the script will need data from the npz files (at least from one of them)
