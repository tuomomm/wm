...
handle.amp = 0
h("objref seclamp")
somaical = saferecord(somasec(0.5)._ref_ica_cal, recvdt)
somaican = saferecord(somasec(0.5)._ref_ica_can, recvdt)
somaicat = saferecord(somasec(0.5)._ref_ica_cat, recvdt)

h(somasec.name()+" seclamp = new SEClamp(0.5)")
h.seclamp.dur1 = 1000
h.seclamp.dur2 = 50
h.seclamp.dur3 = 1000
h.seclamp.amp1 = -110
...
scipy.io.savemat(simstr+'_withSK3_nohotzone_VClamp_r'+str(r_VClamp)+'.mat',{'spikes': spikes, 'Vsoma': array(cell.soma_volt)[int(990/recvdt):int(1060/recvdt)], 'times': [recvdt*i for i in range(int(990/recvdt),int(1060/recvdt))], 'somaical': array(somaical)[int(990/recvdt):int(1060/recvdt)], 'somaican': array(somaican)[int(990/recvdt):int(1060/recvdt)], 'somaicat': array(somaicat)[int(990/recvdt):int(1060/recvdt)]})
