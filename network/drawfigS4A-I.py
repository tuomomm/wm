  ...
  for iiarea in range(0,len(iareas)):
    ...
    for ichannel in range(0,len(channels)):
      ...
      for iseed in range(1,6):
        ...
        filename = 'FRandMUA_sim_net_long_withSK3_'+thisarea+('_' if len(thischannel) > 0 else '')+thischannel+addition+'_altgain_seed'+str(iseed)+'.mat'
        if thischannel == '':
    ...
    for igroup in [0,1]:
      ...
      for iisubj in range(0,len(isubjs)):
        ...
        FRvecs = []
        for myseed in [1]:
          filename = 'subjs/FRandMUA_sim_net_long_withSK3_'+thisarea+'subj'+str(isubj)+addition+'_altgain_seed'+str(myseed)+'.mat'
