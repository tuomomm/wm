...
def drawraster (ld,sz=2):
  ...
  import scipy.io
  FRvecs = [zeros(int(tstop/5)+1,),zeros(int(tstop/5)+1,),zeros(int(tstop/5)+1,),zeros(int(tstop/5)+1,),zeros(int(tstop/5)+1,)]
  ...
  for ispike in range(0,len(lspks)):
    ...
    FRvecs[ivec][int(lspks[ispike]*200)] = FRvecs[ivec][int(lspks[ispike]*200)] + 1
    if ivec == 0:
      if lids[ispike] < 96 or (lids[ispike] >= 256 and lids[ispike] < 328) or (lids[ispike] >= 401 and lids[ispike] < 448) or (lids[ispike] >= 576 and lids[ispike] < 651): #Stimulated half
        FRvecs[3][int(lspks[ispike]*200)] = FRvecs[3][int(lspks[ispike]*200)] + 1
      else: #Non-stimulated half
        FRvecs[4][int(lspks[ispike]*200)] = FRvecs[4][int(lspks[ispike]*200)] + 1
  row = 0
  makeMUA(ld,memlist,nMem,row,tstart,tstop,0.5)
  MUAkeys = list(ld['dMUA0.5'][0].keys())
  MUAs = [array(ld['dMUA0.5'][0][MUAkeys[i]]) for i in range(0,len(MUAkeys))]
  scipy.io.savemat('FR_'+simstr+'_withCaApic_higherres.mat', {'FRvecs': FRvecs, 'Caapics': ld['Adend3_cai']})
