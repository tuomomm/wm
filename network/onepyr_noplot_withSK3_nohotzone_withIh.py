...
for k in ['tstop','iseed','pseed','wseed','saveExtra']:
  if type(dconf[k]) == str:
    dconf[k] = float(dconf[k])
for k in ['simstr', 'saveout', 'saveoutSpikes', 'recdt', 'recvdt', 'verbose', 'indir', 'outdir','saveExtra']:
...
for sdx in [1,0,2,3,4]:
  ...
  for x in lx:
    if len(ca[er].nodes(sec)(x)) > 0:
      ...
      lip3.append(vc)
      vc = Vector(); vc.record(sec(x).iar._ref_m)
      HCNm.append(vc)
...
def savedata (simstr):
  ...
  soma_cai = zeros((ne,szslow)); Adend3_cai = zeros((ne,szslow)); Adend2_cai = zeros((ne,szslow)); Adend3_cai = zeros((ne,szslow))
  ...
  for c in ce:
    ...
    Adend2_cai[cdx,:] = c.Adend2_cai.to_python()
    Adend1_cai[cdx,:] = c.Adend1_cai.to_python()
...
if saveoutSpikes:
  ...
  scipy.io.savemat(simstr+'_nohotzone_withIh.mat',{'spikes': spikes, 'Vsoma': array(cell.soma_volt), 'times': [recvdt*i for i in range(0,len(array(cell.soma_volt)))], 'HCNm': array(HCNm), 'lcai': array(lcai), 'cai': [array(cell.soma_cai),array(cell.Bdend_cai),array(cell.Adend1_cai),array(cell.Adend2_cai),array(cell.Adend3_cai)]})
