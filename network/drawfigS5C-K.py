#cp drawfig3_withSK3_altgain_mildvar.py drawfig3_withSK3_altgain_mildvar_ExpWMnoNM.py
from pylab import *
import scipy.io
import mytools
...
#addition = '_EEGain_0_00025_EIGainFS_0_00025_EIGainLTS_0_00025'
addition = ''
...
for iipop in [0,1,2]:
  ...
  for iiarea in range(0,len(iareas)):
    ...
    for ichannel in range(0,len(channels)):
      ...
      for iseed in range(1,6):
        filename = 'FRandMUA_sim_net_ExpWMnoNM_long_withSK3_'+thisarea+'_'+thischannel+addition+'_altgain_k0_1_seed'+str(iseed)+'.mat'
        if thischannel == '':
          filename = 'FRandMUA_sim_net_ExpWMnoNM_long_withSK3_'+thisarea+addition+'_altgain_k0_1_seed'+str(iseed)+'.mat'
        elif thischannel == 'CTRL':
          filename = 'FRandMUA_sim_net_ExpWMnoNM_long_withSK3_CTRL'+addition+'_altgain_k0_1_seed'+str(iseed)+'.mat'
    ...
    for igroup in [0,1]:
      ...
      FRvecs_all_means = []
      FRthisgroup = []
      printedNotExist = False
      for iisubj in range(0,len(isubjs)):
        isubj = isubjs[iisubj]
        bins_all = []
        FRvecs = []
        for myseed in [1]:
          filename = 'subjs/FRandMUA_sim_net_ExpWMnoNM_long_withSK3_'+thisarea+'subj'+str(isubj)+addition+'_altgain_k0_1_PConly_seed'+str(myseed)+'.mat'
          ...
          if exists(filename):
            ...
            for ibin in range(0,len(FRs)):
              FRs[ibin] = sum(A['FRvecs'][ipop][Nsum*ibin:Nsum*(ibin+1)])/(Ncellsperpop[ipop]*Nsum/100) #Divided by the number of cells in the population, corrected by the bin size (if bin size is 1 s, i.e. Nsum=100, then we have the frequency in Hz without normalization, but if 10 s, i.e. Nsum=1000, then we have to also divide by 10 to get spikes/s)  
            FRvecs.append(FRs[:])
          elif not printedNotExist:
            print(filename+' does not exist')
            printedNotExist = True
  ...
  for iax in range(0,9):
    ...
    f.text(pos.x0 - 0.04, pos.y1 - 0.0, chr(ord('A')+2+iax), fontsize=9)

for iax in [0,1,2]:
  axarr[iax].set_ylim([0,10])
  axarr[3+iax].set_ylim([0,14])
  axarr[6+iax].set_ylim([0,27])
...
axarr[3].set_ylabel('LTS FR (Hz)',fontsize=6)
axarr[6].set_ylabel('PV+ FR (Hz)',fontsize=6)

#Don't draw the bars for PV+ since almost no curves go beyond double the baseline
axnew[6].set_visible(False)    
axnew[7].set_visible(False)    
axnew[8].set_visible(False)    

  
f.savefig("figS5C-K.pdf")
