...
for iay in range(0,3):
  for iax in range(0,3):
    axarr[iay*3+iax].set_position([0.08+0.31*iax,0.78-0.24*iay,0.25,0.18])
    axnew.append(f.add_axes([0.08+0.31*iax+0.21,0.78-0.24*iay+0.08,0.03,0.09]))
...
for iipop in [0,1,2]:
  ...
  for iiarea in range(0,len(iareas)):
    ...
    for ichannel in range(0,len(channels)):
      ...
      if thisarea == 'PFC' and thischannel in ['kap','cagk_ikc'] or thisarea == 'ACC' and thischannel in ['iar_PConly','nax_PConly']:
        continue
      bins_all = []
      FRvecs = []
      for iseed in range(1,6):
      
        filename = 'FRandMUA_sim_net_long_withSK3_'+thisarea+('_' if len(thischannel) > 0 else '')+thischannel+addition+'_adjEEB_altgain_seed'+str(iseed)+'.mat'
        if thischannel == '':
          filename = 'FRandMUA_sim_net_long_withSK3_'+thisarea+addition+'_adjEEB_altgain_seed'+str(iseed)+'.mat'
      ...
      print("istarts = "+str(istarts)+", iends = "+str(iends))
      if len(istarts) > 0 and len(iends) > 0:
        ...
        FRbeyondThr = 0
      axnew[3*iipop].bar(ichannel*(1+iiarea),FRbeyondThr,facecolor='#888888' if thischannel == 'CTRL' else cols[iarea])
      axnew[3*iipop].set_ylabel('time\nabove '+'{:.2f}'.format(FRThr)+'\nHz (s)',fontsize=4.8)
      print('time above\n'+'{:.2f}'.format(FRThr)+' Hz (s)')
      #print('Median time above 2*baseline, igroup='+str(igroup)
        
      FRvecs_all.append(FRvecs[:])
      print("FRbaseline = "+str(FRbaseline)+" FRbeyondThr = "+str(FRbeyondThr))


    axarr[3*iipop].plot(10*Nsum*array(range(0,len(FRs)))/1000-10, [mean([FRvecs[isamp][it] for isamp in range(0,len(FRvecs))]) for it in range(0,len(FRs))],label='SCZ, '+areas[iarea],color=cols[iarea],lw=0.4)
    if iarea == 1:
      FRvecs = FRvecs_all[0]
      axarr[3*iipop].plot(10*Nsum*array(range(0,len(FRs)))/1000-10, [mean([FRvecs[isamp][it] for isamp in range(0,len(FRvecs))]) for it in range(0,len(FRs))],'k-',label='CTRL',lw=0.4)
    ...
    for igroup in [0,1]:
      ...
      for iisubj in range(0,len(isubjs)):
        ...
        baselineFRs = []
        for myseed in [1]:
          filename = 'subjs/FRandMUA_sim_net_long_withSK3_'+thisarea+'subj'+str(isubj)+addition+'_altgain_adjEEB_seed'+str(myseed)+'.mat'
          ...
          if exists(filename):
            ...
            for ibin in range(0,len(FRs)):
              FRs[ibin] = sum(A['FRvecs'][ipop][Nsum*ibin:Nsum*(ibin+1)])/(Ncellsperpop[ipop]*Nsum/100) #Divided by the number of cells in the population, corrected by the bin size (if bin size is 1 s, i.e. Nsum=100, then we have the frequency in Hz without normalization, but if 10 s, i.e. Nsum=1000, then we have to also divide by 10 to get spikes/s)  
            FRvecs.append(FRs[:])
            baselineFRs.append(mean(FRs[3:10]))
          elif iisubj == 0:
            print(filename+' does not exist')
        FRthisgroup.append(FRvecs[:])

        if iisubj%8 == 0:
          axarr[3*iipop+1+iiarea].plot(10*Nsum*array(range(0,len(FRs)))/1000-10, [mean([FRvecs[isamp][it] for isamp in range(0,len(FRvecs))]) for it in range(0,len(FRs))],label=channel_titles[ichannel],color=dimcols[iarea] if igroup==1 else dimgray,lw=0.1)
        FRvecs_all_means.append([mean([FRvecs[isamp][it] for isamp in range(0,len(FRvecs))]) for it in range(0,len(FRs))])
        baselineFRsthisgroup.append(baselineFRs[:])
      FRthisarea.append(FRthisgroup[:])
      groupmeans.append(mean(array(FRvecs_all_means),axis=0))
      baselineFRsthisarea.append(baselineFRsthisgroup[:])

    print('Baselines different? ipop='+str(ipop)+', '+areas[iarea]+' p-value = '+str(scipy.stats.ranksums(array(baselineFRsthisarea[0])[:,0],array(baselineFRsthisarea[1])[:,0])[1])+', medians = '+str(median(array(baselineFRsthisarea[0])[:,0]))+' vs '+str(median(array(baselineFRsthisarea[1])[:,0])))
    FRbeyondThrthisarea = []
    ...
    for igroup in [0,1]:
      ...
      for iisubj in range(0,len(isubjs)):
        ...
        FRThr = FRThrCoeff*FRbaseline
        istarts = [i for i in range(0,len(FRs)-1) if FRmean[i] < FRThr and FRmean[i+1] >= FRThr and i >= 10]
        iends = [i for i in range(0,len(FRs)-1) if FRmean[i] >= FRThr and FRmean[i+1] < FRThr and i >= 10]
        print("istarts = "+str(istarts)+", iends = "+str(iends))

        if len(istarts) > 0 and len(iends) > 0:
          startt = 10*Nsum*(istarts[0]+(FRThr-FRmean[istarts[0]])/(FRmean[istarts[0]+1]-FRmean[istarts[0]]))
          endt = 10*Nsum*(iends[-1]+(FRmean[iends[-1]]-FRThr)/(FRmean[iends[-1]]-FRmean[iends[-1]+1]))
          FRbeyondThrthissubj.append((endt-startt)/1000) #change from milliseconds to second
        elif len(istarts) > 0:
          FRbeyondThrthissubj.append(inf)
        else:
          FRbeyondThrthissubj.append(0)
        FRbeyondThrthisgroup.append(FRbeyondThrthissubj[:])
      FRbeyondThrthisarea.append(FRbeyondThrthisgroup[:])

    axnew[3*iipop].set_xticks([])
    for igroup in [0,1]:
      axarr[3*iipop+1+iiarea].plot(10*Nsum*array(range(0,len(FRs)))/1000-10,groupmeans[igroup],lw=0.75,color=cols[iarea] if igroup else '#000000',label=thisarea)
    
      mybar(axnew[3*iipop+1+iiarea],igroup,FRbeyondThrthisarea[igroup],facecolor=cols[iarea] if igroup else '#888888')
      if max(array(FRbeyondThrthisarea[igroup])) > 90:
        drawdiscontinuity(axnew[3*iipop+1+iiarea],89,5,x=igroup,xoffset=0.15,lw=0.6,lw2=0.3)
      print('Median time above 2*baseline, iipop='+str(iipop)+', iiarea='+str(iiarea)+', igroup='+str(igroup)+': '+str(median(FRbeyondThrthisarea[igroup])))

    pval = scipy.stats.ranksums(FRbeyondThrthisarea[0],FRbeyondThrthisarea[1])[1]
    if pval < 0.05:
      axnew[3*iipop+1+iiarea].plot([0,0,1,1],[105,110,110,105],'k-',lw=0.1)
      axnew[3*iipop+1+iiarea].plot(0.5,115,'k*',mew=0.9,lw=0.9,ms=3)
    else:
      axnew[3*iipop+1+iiarea].plot([0,0,1,1],[105,110,110,105],'k-',lw=0.1,color='#FFFFFF')
      axnew[3*iipop+1+iiarea].plot(0.5,115,'k*',mew=0.9,lw=0.9,ms=3,color='#FFFFFF')
    
    print("iipop="+str(iipop)+" iarea="+str(iarea)+" "+areas[iarea]+" p-val = "+str(pval))

    axnew[3*iipop+1+iiarea].set_ylabel('time\nabove '+'{:.2f}'.format(FRThr)+'\nHz (s)',fontsize=4.8)
    axnew[3*iipop+1+iiarea].set_xticks([])
    axnew[3*iipop+1+iiarea].set_xlim([-0.5,1.5])
    axnew[3*iipop+1+iiarea].set_ylim([-5,125])
    axnew[3*iipop+1+iiarea].set_yticks([0,80])
  axnew[3*iipop].set_xlim([-0.5,2.5])
  for iax in range(0,9):
...
axarr[3].set_ylim([0,10.0])
axarr[6].set_ylim([0,20.0])

axarr[0].set_title('Population-averaged models', fontsize=6)
axarr[1].set_title('Subject-wise ACC models', fontsize=6)
axarr[2].set_title('Subject-wise PFC models', fontsize=6)
handles, labels = axarr[0].get_legend_handles_labels()
l=axarr[0].legend([handles[idx] for idx in [1,0,2]],[labels[idx] for idx in [1,0,2]],fontsize=4,loc=(0.26,0.69))
axarr[0].set_ylabel('Stimulated PCs',fontsize=6)
axarr[3].set_ylabel('LTS',fontsize=6)
axarr[6].set_ylabel('PV+',fontsize=6)
