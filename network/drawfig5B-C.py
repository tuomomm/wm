...
axlabels = ['No covariates','Only SCZ','Only HC']
ithr = 9
...
axarr[0].set_position([0.04,       0.12+0.44+0.24,0.19,0.15])
axarr[1].set_position([0.04+0.20,  0.12+0.44+0.24,0.19,0.15])
axarr[2].set_position([0.04+0.20*2,0.12+0.44+0.24,0.19,0.15])
axarr[0].set_visible(False)
axarr[3].set_visible(False)

for iax in range(1,3):
  if iax == 1: 
    A = scipy.io.loadmat('../genetic/corrs_ztransformed_LNSonly_againstAgeSex_SZ.mat')
  elif iax == 2: 
    A = scipy.io.loadmat('../genetic/corrs_ztransformed_LNSonly_againstAgeSex_HC.mat')
  ...
  for iPRS in range(0,len(PRSnames_to_consider)):
    ...
    if iPRSname == -1:
      ...
      exit()
    axarr[iax].bar(iPRS,corrs[iPRSname][0][ithr],facecolor=cols[iPRS],hatch=hatches[iPRS],edgecolor=edgecolors[iPRS],linewidth=0,label=PRSnames[iPRS])
    ...
    axarr[iax].text(iPRS,min(0,corrs[iPRSname][0][ithr])-0.001,'p='+'{:.3f}'.format(betas_pvals[iPRSname][ithr][1][1])+('$\\ast\!\\ast$' if betas_pvals[iPRSname][ithr][1][1] < 0.05/14 else ('$\\ast$' if betas_pvals[iPRSname][ithr][1][1] < 0.05 else '')), fontsize=6,rotation=90,ha='center',va='top',fontweight=fontweight)

  axarr[iax].set_xlim([-0.6,13.6])
  axarr[iax].plot([-1,14],[0,0],'k-',lw=0.3)
  axarr[iax].set_xticks([])
  axarr[iax].set_ylim([-0.15,0.01])
  axarr[iax].text(2.03,0.045,axlabels[iax],ha='left',va='top',fontsize=7)
for iax in range(1,3):
  pos = axarr[iax].get_position()
  f.text(pos.x0 - 0.01, pos.y1 +0.01, chr(ord('A')+iax), fontsize=11)
axarr[1].set_ylabel('Corr. coeff.',fontsize=6,labelpad=1)
axarr[2].set_yticklabels([])
  
f.savefig("fig5B-C.pdf")
