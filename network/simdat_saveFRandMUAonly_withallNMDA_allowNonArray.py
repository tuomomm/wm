...
def loaddat (simstr,nhost,quiet=False):
  ...
  vt = ld[0]['vt'][:]
  locvarlist = []
  for loc in ['soma','Adend3','Adend2','Adend1','Bdend']:
  ...
  for k in locvarlist:
    ...
    for pcid in range(nhost):
      ...
      if k == 'soma_iNM': # special case for iNM of I cells, added by Tuomo 19.2.2026
        lI=ld[pcid]['lI']; dI=ld[pcid]['soma_iNM_i']; row=0
        for row,cdx in enumerate(lI): ldk[cdx,:] = dI[row,0:sz]
  for pcid in range(nhost):
    del ld[pcid].f
    ld[pcid].close()
...
def drawraster (ld,sz=2):
  ...
  try:
    NMDAcurrs = [ld['soma_iNM'],ld['Bdend_iNM'],ld['Adend1_iNM'],ld['Adend2_iNM'],ld['Adend3_iNM']]
  except:
    NMDAcurrs = [[],[],[],[],[]]
  ...
  try:
    ...
    scipy.io.savemat('FR_'+simstr+'.mat', {'FRvecs': FRvecs})
    return 0
  MUAkeys = list(ld['dMUA10.0'][0].keys())
  try:
    MUAs = [array(ld['dMUA10.0'][0][MUAkeys[i]]) for i in range(0,len(MUAkeys))]
  except:
    MUAs = [ld['dMUA10.0'][0][MUAkeys[i]] for i in range(0,len(MUAkeys))]
  scipy.io.savemat('FRandMUA_'+simstr+'.mat', {'FRvecs': FRvecs, 'MUAkeys': MUAkeys, 'MUAs': MUAs, 'NMDAcurrs': NMDAcurrs})
