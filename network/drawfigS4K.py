...
axarr[0].set_position([0.076,0.66,0.41,0.15])
axnew = f.add_axes([0.4,0.67,0.07,0.05])
boxoff(axnew,'bottom')
axnew.set_xticks([])
  ...
  for iiarea in [1,0]:
    ...
    for ichannel in range(0,len(channels)):
      ...
      for iseed in range(1,11):
        ...
        filename = 'FRandMUA_sim_net_long_withSK3_'+thisarea+'_'+thischannel+addition+'_altgain_withNMDA_seed'+str(iseed)+'.mat' #'_withCa_higherres.mat'
        if thischannel == '':
          filename = 'FRandMUA_sim_net_long_withSK3_'+thisarea+addition+'_altgain_withNMDA_seed'+str(iseed)+'.mat' #'_withCa_higherres.mat'
        elif thischannel == 'CTRL':
          filename = 'FRandMUA_sim_net_long_withSK3_CTRL'+addition+'_altgain_withNMDA_seed'+str(iseed)+'.mat' #'_withCa_higherres.mat'
        ...
        Casomas = A['NMDAcurrs'][0]+A['NMDAcurrs'][1]+A['NMDAcurrs'][2]+A['NMDAcurrs'][3]+A['NMDAcurrs'][4] #Sum up the NMDA currents. They are all in nA.
        meanCaTimeCourse = mean([Casomas[i,:] for i in range(0,Casomas.shape[0]) if Casomas[i,-1] != 0],axis=0)
    ...
    print(str(CasomaMeans_all[0])+' '+str(CasomaMeans_all[1]))
    ...
    if pval < 0.05:
      axnew.plot([iiarea,iiarea,2,2],[x-350*(iiarea==0) for x in [-1500,-1600,-1600,-1500]],'k-',lw=0.2)
      axnew.text(mean([iarea,2]),-1900-350*iiarea,'*',fontsize=5,ha='center',va='center')

axarr[0].set_xlim([-9,90])
handles, labels = axarr[0].get_legend_handles_labels()
l=axarr[0].legend([handles[idx] for idx in [1,0,2]],[labels[idx] for idx in [1,0,2]],fontsize=4,ncol=3,loc='lower left')
...
f.text(pos.x0 - 0.06, pos.y1 - 0.01, chr(ord('K')+0), fontsize=12)

axnew.xaxis.tick_top()
axnew.xaxis.set_label_position('top')


f.savefig("figS4K.pdf")
