...
filename_seed1 = 'sim_onepyr_withSK3_seed1_nohotzone_withIh'
...
givennames = ['withSK3_95apamin_nohotzone','withSK3_fullapamin_nohotzone','withSK3_apamin_nohotzone','withSK3_nohotzone']
...
for iSK in [0,1,2,3]:
  ...
  fs_all.append(fs[:])
  if iSK == 3:
    axarr[1].plot(amps_plotted,fs,'r-',lw=0.5,color='#000000')
  elif iSK == 2:
    axarr[1].plot(amps_plotted,fs,'r-',lw=0.5,color='#FF8888')
  elif iSK == 1:
...
l2 = mytools.mylegend(f,[0.59,0.785,0.165,0.0375],['k-','r-'],['sim. data, 98% block of SK','sim. data, CTRL'],nx=1,dx=1,yplus=0.5,yplustext=0.35,colors=['#FF8888','#888888'],linewidths=[0.4,0.4],myfontsize=5.5)
l2.set_xlim([0.1,3.4])
for axis in ['top','bottom','left','right']:
...
A = scipy.io.loadmat('15dec31_1_withSK3_nohotzone_VClamp_r0.02.mat')
spikes = A['spikes']
times = A['times']
somaical = A['somaical']
somaican = A['somaican']
...
for icomp in range(0,5):
  ...
  axarr[2].plot((0.1*binwidth*array(range(0,len(binslcai_all[0][0])))-10000)/1000,[mean([binslcai_all[iseed][icomp][it] for iseed in range(0,len(binslcai_all))]) for it in range(0,len(binslcai_all[0][icomp]))],lw=0.4,color=cols_compartments[icomp],label=compname)
  axnew.plot((0.1*binwidth*array(range(0,len(binslcai_all[0][0])))-10000)/1000,[mean([binslcai_all[iseed][icomp][it] for iseed in range(0,len(binslcai_all))]) for it in range(0,len(binslcai_all[0][icomp]))],lw=0.4,color=cols_compartments[icomp],label=compname)
...
for iax in range(0,6):
  ...
  f.text(pos.x0 - 0.09, pos.y1 - 0.01, chr(ord('A')+iax), fontsize=12)
