...
for iWMgroup in range(0,len(WMmeasures_all)):
  ...
  for iattr in range(0,len(attrs)):
    ...
    Ns_thisattr = []
    if not exists('assoc/BDPRSs_othermeasures_ztransformed_LNSonly_diags_'+myattr+'.mat'):
      print('assoc/BDPRSs_othermeasures_ztransformed_LNSonly_diags_'+myattr+'.mat does not exist')
      continue
    MAT_WM = scipy.io.loadmat('assoc/BDPRSs_othermeasures_ztransformed_LNSonly_diags_'+myattr+'.mat')
    ...
    if group == 'HC':
      ...
      print('Group unknown!')
    if len(MAT_WM['group_vec']) == 0:
      print("""MAT_WM['group_vec']) = 0, continuing""")
      continue
...
scipy.io.savemat('corrsBD_ztransformed_LNSonly_againstAgeSex_'+group+'.mat',{'corrs': corrs,'betas_pvals': betas_pvals, 'Ns': Ns, 'attrs': attrs_saved_all})
qwe
