...
for iWMgroup in range(0,len(WMmeasures_all)):
  ...
  for iattr in range(0,len(attrs)):
    ...
    for ithr in range(0,len(thrs)):
      ...
      Xdf = pandas.DataFrame(numpy.array([PRSs,ages]).T,columns = ['PRS','Age'])
...
scipy.io.savemat('corrs_ztransformed_LNSonly_againstAge_'+group+'.mat',{'corrs': corrs,'betas_pvals': betas_pvals, 'Ns': Ns, 'attrs': attrs_saved_all})
qwe
