...
for ithr in range(0,len(thrs)):
  mythr = thrs[ithr]
  if not exists('genetic/'+genelist+'_'+str(mythr)+'_PRS.best'):
    PRS_vecs_all.append([])
    continue
  #qwe
  input_file = open('genetic/'+genelist+'_'+str(mythr)+'_PRS.best','r')
...
scipy.io.savemat('assoc/PRSs_othermeasures_ztransformed_LNSonly_diags_'+genelist+'.mat', {'WMmeasures_vec': WMmeasures_vec, 'age_vec': age_vec, 'group_vec': group_vec, 'sex_vec': sex_vec, 'subjects': subject_vec, 'PRS_vecs': PRS_vecs_all, 'thrs': thrs, 'IQ_vec': IQ_vec, 'days_since_2000_vec': days_since_2000_vec, 'educat_vec': educat_vec})
