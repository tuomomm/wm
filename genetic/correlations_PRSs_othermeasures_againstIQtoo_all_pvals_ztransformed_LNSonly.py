...
for iWMgroup in range(0,len(WMmeasures_all)):
  ...
  for iattr in range(0,len(attrs)):
    ...
    for ithr in range(0,len(thrs)):
      ...
      Xdf = pandas.DataFrame(numpy.array([PRSs,ages,sexes,IQs]).T,columns = ['PRS','Age','Sex','IQ'])
...
scipy.io.savemat('corrs_ztransformed_LNSonly_againstAgeSexIq_'+group+'.mat',{'corrs': corrs,'betas_pvals': betas_pvals, 'Ns': Ns, 'attrs': attrs_saved_all})
qwe
